...
function Menu(caller, options) {
    ...
    this.chooseItem = function (item) {
        ...
        var selected_cat = $(item).text();
        if ($(item).parents('li').parents('li').length) {
            var parent_cat = $(item).parents('li').parents('li').children('a').text();
            $('#menuSelection').html(parent_cat + ' <i class="icon fa-chevron-right margin-left-4 margin-right-4 font-size-10"></i> ' + selected_cat);
        } else {
            $('#menuSelection').text(selected_cat);
        }
        $('#menuSelectedId').val($(item).data('id'));
        $('#menuLog label').remove();
