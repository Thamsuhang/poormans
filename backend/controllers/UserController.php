<?php
    ...
    class UserController extends Controller {
        ...
        public function actionIndex() {
            return $this->redirect(Yii::$app->request->baseUrl . '/' . Yii::$app->params['user_role'][Yii::$app->user->identity->role].'/profile');
        ...
        public function actionAdministrators() {
            if (Yii::$app->user->isGuest
                || !in_array(Yii::$app->params['role_user'][Yii::$app->session[Yii::$app->params['user_session']]->role], $this->critical)
            ) {
        ...
        public function actionDeleteProfilePicture() {
            if (Misc::delete_file($_POST['img'], 'profile')) {
                HUser::editableUser('', Yii::$app->session[Yii::$app->params['user_session']]->id, 'profile_picture');
                echo json_encode(TRUE);
                die;
            }
            echo json_encode(FALSE);
        ...
        public function actionCrop() {
            if ($return = Misc::base64_to_jpeg($_POST['image_data'], Yii::$app->session[Yii::$app->params['user_session']]->username, 'profile')) {
                $user = HUser::getUser('id', Yii::$app->session[Yii::$app->params['user_session']]->id);
                if (!empty($user['profile_picture'])) {
                    Misc::delete_file($user['profile_picture'], 'profile');
                }
                Misc::delete_file($_POST['image_name'], 'profile');
                HUser::editableUser($return, Yii::$app->session[Yii::$app->params['user_session']]->id, 'profile_picture');
                echo json_encode($return);
                die;
            }
            echo json_encode(FALSE);
        ...
        public function actionGetUsersForAutocomplete() {
            ...
            if ($users != NULL) {
                foreach ($users as $key => $value) {
                    if ($value['id'] != Yii::$app->session[Yii::$app->params['user_session']]->user_id) {
